%% nonlinear_rdatasets_cases_6.m
% Nonlinear regression on 6 selected cases from the Rdatasets repository.
...
%% 4. Penguins Adelie
mask = strcmp(string(Tpenguins.species), 'Adelie') & ...
...
caseResults{caseId} = fit_case(Tpenguins, mask, 'flipper_length_mm', 'body_mass_g', ...
    'Penguins Adelie', 'power', [1;1.5], [0;0], [1e6;5], []);

%% 5. MPG pickup
mask = strcmp(string(Tmpg.class), 'pickup') & ...
    isfinite(Tmpg.displ) & isfinite(Tmpg.hwy);
caseId = caseId + 1;
caseResults{caseId} = fit_case(Tmpg, mask, 'displ', 'hwy', ...
    'MPG pickup', 'expdecay', [15;20;0.5], [0;0;0], [100;100;10], []);
...
write_summary_csv(summaryTable, fullfile(outDir, 'matlab_nonlinear_summary_6cases.csv'));
make_overview_figure(cases, fullfile(outDir, 'matlab_nonlinear_cases_overview_6cases'));
%make_overview_figure(cases, fullfile(outDir, 'matlab_nonlinear_cases_overview_6cases.png'));
make_diagnostic_figure(cases, fullfile(outDir, 'matlab_nonlinear_diagnostics_6cases'));
%make_diagnostic_figure(cases, fullfile(outDir, 'matlab_nonlinear_diagnostics_6cases.png'));
